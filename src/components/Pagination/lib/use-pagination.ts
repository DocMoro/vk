...
export const usePagination = ({ currentPage, siblingCount = 1, totalPageCount }: Props): ('...' | number)[] => {
  return useMemo(() => {
    ...
    const rightSiblingIndex = currentPage + siblingCount >= totalPageCount ? totalPageCount : currentPage + siblingCount
